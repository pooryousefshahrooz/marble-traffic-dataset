...
class WerewolfAgent:
    ...
    def __init__(
        ...
        # Initialize the API client. verify=False: the local TLS proxy
        # (scripts/tls_ollama_proxy.py) uses a self-signed research-lab
        # cert -- traffic is still genuinely TLS-encrypted end to end, we
        # just don't validate the cert chain for this fixed loopback proxy.
        self.client = OpenAI(
            base_url=self.base_url,
            api_key=self.api_key,
            http_client=httpx.Client(verify=False) if self.base_url.startswith("https://127.0.0.1") else None,
        )
