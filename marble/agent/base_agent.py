...
class BaseAgent:
    ...
    def _calculate_token_usage(self, task: str, result: str) -> int:
        ...
        """
        # A chain hop can legitimately produce no result (e.g. the agent
        # only forwarded the task without generating new content) -- treat
        # a missing task/result as contributing 0 tokens rather than
        # crashing the whole simulation.
        token_count = (len(task or "") + len(result or "")) // 4
        return token_count
