...
class EnginePlanner:
    ...
    def decide_next_step(self, agents_results: List[Dict[str, Any]]) -> bool:
        ...
        try:
            ...
            # json_parse() raises ValueError (wrapping JSONDecodeError), not
            # JSONDecodeError itself -- smaller local models are also more
            # likely to drop the requested JSON format entirely than the
            # larger models the paper evaluated with.
