...
class AgentGraph:
    ...
    def get_agent_profiles_linked(self, agent_id: str) -> Dict[str, Dict[str, Any]]:
        """
        Used by chain coordination to get candidate hand-off targets for the
        given agent. Intended to restrict to topologically-linked neighbors,
        but this dataset's `relationships` field is a flat, fully-connected
        "parent" edge set that is identical across all 4 topology configs
        (graph/star/tree/chain) and unrelated to coordination topology --
        there is no real per-topology subgraph to filter on here. `get_children`
        (adjacency_list-based) is also always empty since adjacency_list is
        only ever populated by add_agent(), which config-driven graph
        construction never calls. So this returns all other agents, same as
        get_agent_profiles(), matching how graph/star/tree already hand
        every agent unrestricted profiles via engine._get_agent_profiles().

        Args:
            agent_id (str): The ID of the requesting agent (excluded from the result).

        Returns:
            Dict[str, Dict[str, Any]]: Profiles of all other agents.
        """
        return {
            aid: profile
            for aid, profile in self.get_agent_profiles().items()
            if aid != agent_id
        }
