...
class Evaluator:
    ...
    def evaluate_communication(self, task: str, communications: str) -> None:
        ...
        # Fill in the placeholders {task} and {communications}. Templates
        # embed literal example JSON (e.g. {"rating": 4}), which collides
        # with str.format()'s brace syntax -- substitute the two known
        # placeholders textually instead of calling .format() on the whole
        # template.
    ...
    def evaluate_planning(self, summary: str, agent_profiles: str, agent_tasks: str, results: str) -> None:
        ...
        # Fill in the placeholders. Same literal-JSON-brace issue as
        # evaluate_communication() above -- substitute textually, not via
        # .format() on the whole template.
        prompt = (
            planning_prompt_template
            .replace("{summary}", summary)
            .replace("{agent_profiles}", agent_profiles)
            .replace("{agent_tasks}", agent_tasks)
